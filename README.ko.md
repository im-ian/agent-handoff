...
Claude Code에서는 슬래시 명령어로 쓸 수 있고, Codex는 같은 `handoff` CLI를 `--profile codex`로 사용할 수 있습니다.
...
다른 머신에서도 동일 설치 + `/agent-handoff:init` 후:
...
Claude Code에서 `/agent-handoff:status` 실행해서 "command not found" 없이 나오면 완료. 터미널에서 `handoff --version`은 `1.0.0`을 출력.
...
- **`/agent-handoff:push`에서.** `--dry-run` preflight로 finding을 먼저 보여준 뒤 `AskUserQuestion`으로 skip/allow/abort 선택. Public/unknown 가시성 hub는 추가 경고 표시.
...
`~/.agent-handoff/config.json` — 전체 스키마는 [`docs/DESIGN.md`](docs/DESIGN.md) 참조. 대부분은 이 파일을 직접 만질 일 없음 — `/agent-handoff:init`이 합리적인 기본값을 써줍니다.
