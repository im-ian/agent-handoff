...
- If `~/.agent-handoff/config.json` is missing → `/agent-handoff:init` first.
- If no manifest exists for this device → suggest `/agent-handoff:deps add <name> --darwin "..." --linux "..."` to declare deps first.
